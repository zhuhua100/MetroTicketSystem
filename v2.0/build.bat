...
chcp 65001 > nul
echo ========================================
echo   Metro Ticket System v2.0 - 编译脚本
echo   EasyX图形版
echo ========================================
echo.

set "PROJ_DIR=d:\桌面\MetroTicketSystem\v2.0"
...
set "EASYX_DIR=C:\Program Files (x86)\EasyX"

set "INCLUDE=%VC_DIR%\include;%KITS_DIR%\ucrt;%KITS_DIR%\um;%KITS_DIR%\shared;%EASYX_DIR%\include"
set "LIB=%VC_DIR%\lib\x64;%KITS_LIB%\ucrt\x64;%KITS_LIB%\um\x64;%EASYX_DIR%\lib\x64"

set "CL_EXE=%VC_DIR%\bin\Hostx64\x64\cl.exe"

echo 编译中...
"%CL_EXE%" /utf-8 /Fe:%PROJ_DIR%\MetroTicketSystem_v2.exe /I %PROJ_DIR%\include /I "%EASYX_DIR%\include" %PROJ_DIR%\src\main.c %PROJ_DIR%\src\welcome.c %PROJ_DIR%\src\menu.c %PROJ_DIR%\src\file.c %PROJ_DIR%\src\instruction.c %PROJ_DIR%\src\station.c %PROJ_DIR%\src\price.c %PROJ_DIR%\src\ticket.c %PROJ_DIR%\src\payment.c %PROJ_DIR%\src\history.c %PROJ_DIR%\src\search.c %PROJ_DIR%\src\test.c %PROJ_DIR%\src\map.c /link /LIBPATH:"%EASYX_DIR%\lib\x64" EasyXa.lib EasyXw.lib

if %errorlevel% equ 0 (
    echo.
    echo ========================================
    echo  编译成功！生成 MetroTicketSystem_v2.exe
    echo ========================================
) else (
    echo.
    echo ========================================
